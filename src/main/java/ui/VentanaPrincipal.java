...
public class VentanaPrincipal extends JFrame {
	...
	private void buscarMaletas() {
		try {
			...
			// Calcular volumen y encontrar el mínimo volumen posible
			double volumenMinimo = coincidencias.stream()
					.mapToDouble(m -> m.getLargo() * m.getAncho() * m.getAlto())
					.min()
					.orElse(Double.MAX_VALUE);

			// Filtrar solo las maletas con el volumen mínimo
			List<Maleta> seleccionadas = coincidencias.stream()
					.filter(m -> Math.abs(m.getLargo() * m.getAncho() * m.getAlto() - volumenMinimo) < 0.001)
					.collect(Collectors.toList());

			// Mostrar resultados
			StringBuilder sb = new StringBuilder();
			if (seleccionadas.size() == 1) {
				sb.append("Maleta encontrada (menor volumen posible):\n");
			} else {
				sb.append("Maletas encontradas (menor volumen posible):\n");
			}
			for (Maleta m : seleccionadas) {
				sb.append("Ref: ").append(m.getRefMaleta())
						.append(" | Proveedor: ").append(m.getProveedor())
						.append(" | Largo: ").append(m.getLargo())
						.append(" | Ancho: ").append(m.getAncho())
						.append(" | Alto: ").append(m.getAlto())
						.append("\n");
			}
